# img/vendor_dlkm.py
# ╔════════════════════════════════════════════════════════════════╗
# ║  Project: MIA Core - Vendor Dlkm Image Handler                ║
# ║  Link: t.me/FrontendVSCode                                    ║
# ║  Author: Frontend & LED (𝙰𝚕𝚒𝚊 𝙴𝚝𝚑𝚎𝚛 𖤍) 🌷                  ║
# ║  lang: python                                                 ║
# ║  [MIA-HASH-01] ΞΩ77Λβ99PPHD8A71                               ║
# ║  build: 3.10.15                                               ║
# ║  files: vendor_dlkm.py                                        ║
# ╚════════════════════════════════════════════════════════════════╝
...
def print_info(image_name, miatool_output):
    """Вывод информации для vendor_dlkm.img"""
    ...
    if not miatool_output or "Footer version:" not in miatool_output:
        ...
        if RICH_AVAILABLE:
            console.print()
            console.print(Panel(Text(f"📦 VENDOR_DLKM ОБРАЗ: {image_name}",
            ...
            if not avb_info:
                console.print(Panel(
                    "[yellow]⚠️ VENDOR_DLKM — раздел с динамически загружаемыми модулями ядра[/yellow]\n"
                    "Содержит .ko файлы для конкретного вендора.",
                    border_style="yellow"
                ))
        else:
            print(f"\n  {BOLD}{MAGENTA}📦 VENDOR_DLKM ОБРАЗ: {image_name}{RESET}")
            print(f"  {'═' * 50}")
            print(f"\n  {BOLD}Размер:{RESET} {GREEN}{format_size(file_size)}{RESET}")
            if avb_info:
                print(f"  {BOLD}AVB Magic:{RESET} {GREEN}{avb_info['magic']}{RESET}")
                print(f"  {BOLD}AVB Version:{RESET} {avb_info['version_major']}.{avb_info['version_minor']}")
            else:
    ...
    os_version = re.search(r'com\.android\.build\.vendor_dlkm\.os_version\s+->\s+\'([^\']+)\'', miatool_output)
    fingerprint = re.search(r'com\.android\.build\.vendor_dlkm\.fingerprint\s+->\s+\'([^\']+)\'', miatool_output)
    security_patch = re.search(r'com\.android\.build\.vendor_dlkm\.security_patch\s+->\s+\'([^\']+)\'', miatool_output)

    if RICH_AVAILABLE:
        console.print()
        console.print(Panel(Text(f"📦 VENDOR_DLKM ОБРАЗ: {image_name}", style="bold magenta"), border_style="magenta"))
        ...
        if partition_name:
            table_hashtree = Table(title="🌳 HASHTREE (DM-VERITY)", border_style="cyan")
            table_hashtree.add_column("Параметр", style="bold white")
            table_hashtree.add_column("Значение", style="green")

            if dm_version:
                table_hashtree.add_row("Версия dm-verity", dm_version.group(1))
            if partition_name:
                table_hashtree.add_row("Имя раздела", f"[bold green]{partition_name.group(1)}[/bold green]")
            if hash_alg:
                table_hashtree.add_row("Хеш алгоритм", f"[yellow]{hash_alg.group(1)}[/yellow]")
            if image_size:
                table_hashtree.add_row("Размер данных", format_size(image_size.group(1)))
            if tree_offset:
                table_hashtree.add_row("Смещение дерева", tree_offset.group(1))
            if tree_size:
                table_hashtree.add_row("Размер дерева", format_size(tree_size.group(1)))
            if data_block:
                table_hashtree.add_row("Блок данных", f"{data_block.group(1)} bytes")
            if hash_block:
                table_hashtree.add_row("Блок хешей", f"{hash_block.group(1)} bytes")
            if fec_roots:
                table_hashtree.add_row("FEC корней", fec_roots.group(1))
            if fec_offset:
                table_hashtree.add_row("FEC смещение", fec_offset.group(1))
            if fec_size:
                table_hashtree.add_row("FEC размер", format_size(fec_size.group(1)))
            if salt:
                salt_val = salt.group(1)
                table_hashtree.add_row(
        ...
        if os_version:
            print(f"\n  {BOLD}Android OS:{RESET} {GREEN}{os_version.group(1)}{RESET}")
        if security_patch:
