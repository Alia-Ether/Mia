# img/sspm.py
# ╔════════════════════════════════════════════════════════════════╗
# ║  Project: MIA Core - SSPM Image Handler                       ║
...
def detect_format(data):
    if data.startswith(b'\x7fELF'):
        return "ELF"
    if data[:4] == b'SSPM' or b'sspm' in data[:0x100].lower():
        return "SSPM Firmware"
    if data[:4] == b'TINY':
...
def print_info(image_name, _mia_output=None):
    ...
    if RICH_AVAILABLE:
        ...
        table.add_row("Назначение", "Subsystem Power Manager")
        console.print(table)
        ...
        desc_table = Table(title="ℹ️ ЧТО ТАКОЕ SSPM", title_style="bright_red", border_style="cyan")
        ...
        desc_table.add_row("Функция", "Управление питанием подсистем")
